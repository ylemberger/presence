...
6. **מערכת שכר (קריאה בלבד):** `SALARY_SUPABASE_URL` ו-`SALARY_SUPABASE_ANON_KEY` ב-`.env.local` וגם ב-Vercel של **presence**. מפתחות מ-Supabase של השכר → Settings → API (`anon`, בלי `NEXT_PUBLIC_`). הסנכרון רק `SELECT` מ-`salary_records`. לאכיפה במסד השכר הריצי שם [`supabase/salary_read_only_rls.sql`](supabase/salary_read_only_rls.sql).
